(function () {
  ...
  function sound(name){if(!audioState.sfx)return;var notes={click:[[520,.07,'sine',.035,0]],spin:[[210,.11,'triangle',.06,0],[280,.11,'triangle',.06,.1],[360,.14,'triangle',.06,.2]],land:[[660,.12,'sine',.07,0],[880,.22,'sine',.07,.1]],correct:[[523,.13,'triangle',.08,0],[659,.13,'triangle',.08,.09],[784,.27,'triangle',.09,.18]],wrong:[[220,.18,'sawtooth',.045,0],[165,.28,'sawtooth',.04,.13]],message:[[740,.09,'sine',.05,0],[988,.12,'sine',.04,.08]],unlock:[[523,.14,'triangle',.07,0],[659,.14,'triangle',.07,.1],[784,.14,'triangle',.08,.2],[1047,.36,'sine',.08,.3]],victory:[[392,.12,'triangle',.07,0],[523,.12,'triangle',.08,.1],[659,.14,'triangle',.08,.2],[784,.4,'sine',.1,.32]]};(notes[name]||notes.click).forEach(function(n){tone.apply(null,n);});}
  ...
  function renderMatchWheel(match,state,q,category,mine,mySeals,otherSeals,otherName) {
    ...
    container.innerHTML='<style>'+styles+gameStyles+'</style><div class="fg-play fg-spin-scene"><div class="fg-arena-top"><button class="fg-round-btn" id="fgReturn" aria-label="Voltar"><i class="ti ti-arrow-left"></i></button><div><span>RODADA '+(state.rodada||1)+'</span><b>Duelo clínico</b></div><span class="fg-turn mine">SUA VEZ</span></div><section class="fg-versus"><div><span class="fg-duel-avatar">'+esc(initials(s.nome))+'</span><b>Você</b><small>'+mySeals.length+' áreas</small></div><strong>'+mySeals.length+' <i>×</i> '+otherSeals.length+'</strong><div><span class="fg-duel-avatar opponent">'+esc(initials(otherName))+'</span><b>'+esc(otherName)+'</b><small>'+otherSeals.length+' áreas</small></div></section><section class="fg-wheel-stage"><div class="fg-wheel-copy"><span>SUA VEZ</span><h2>Gire a roleta clínica</h2><p>A área sorteada define o desafio desta rodada.</p></div><div class="fg-wheel-shell" id="fgWheelShell"><i class="fg-wheel-pointer"></i><div class="fg-wheel" id="fgWheel">'+wheelItems+'<button id="fgSpin"><small>TOQUE PARA</small>GIRAR</button></div></div><div class="fg-wheel-result" id="fgWheelResult"><i class="ti ti-sparkles"></i><span>Pronto para o desafio?</span></div><button class="fg-spin-button" id="fgSpinButton"><i class="ti ti-refresh"></i> GIRAR ROLETA</button></section></div>';
    ...
    function spin(){
      ...
      if(button.disabled)return; button.disabled=true; center.disabled=true; sound('spin'); shell.classList.add('spinning'); result.innerHTML='<i class="ti ti-loader-2"></i><span>Sorteando área...</span>';
      ...
      setTimeout(function(){sessionStorage.setItem('fg-wheel-'+match.id+'-'+(state.rodada||1),'1');renderMatch(match);},4700);
    }
