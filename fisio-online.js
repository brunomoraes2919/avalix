(function () {
  ...
  function renderMatchWheel(match,state,q,category,mine,mySeals,otherSeals,otherName) {
    ...
    var wheelItems=GAME_CATEGORIES.map(function(c,i){return '<span style="--i:'+i+'"><i class="ti '+icons[i]+'"></i><small>'+esc(c.name.split(' ')[0])+'</small></span>';}).join('')+'<span class="fg-crown-slice" style="--i:6"><i class="ti ti-crown"></i><small>Coroa</small></span>';
    ...
    function spin(){
      ...
      var crown=Math.random()<1/7,index=crown?6:Math.max(0,GAME_CATEGORIES.findIndex(function(c){return c.id===q.cat;}));
      requestAnimationFrame(function(){wheel.style.transform='rotate('+(2520-index*(360/7))+'deg)';});
  ...
  function renderMatchResult(container,match) {
    ...
    container.innerHTML='<style>'+styles+resultStyles+'</style><section class="fg-result-screen '+(won?'winner':'loser')+'">'+confetti+fireworks+'<div class="fg-result-glow"></div><div class="fg-result-card"><span class="fg-result-kicker">'+(won?'PARTIDA CONCLUÍDA · VITÓRIA':'PARTIDA CONCLUÍDA')+'</span><div class="fg-result-mascot"><img src="'+mascot+'" alt=""></div><div class="fg-result-copy"><h1>'+(won?'Você venceu!':'Não foi dessa vez')+'</h1><p>'+(won?'Parabéns! Você conquistou as seis áreas da fisioterapia e venceu o duelo clínico.':esc(winnerName)+' completou as seis áreas primeiro. Revise os desafios e prepare-se para a revanche!')+'</p><div class="fg-result-score"><div><small>VOCÊ</small><b>'+mySeals.length+'</b><span>áreas</span></div><strong>×</strong><div><small>ADVERSÁRIO</small><b>'+otherSeals.length+'</b><span>áreas</span></div></div><div class="fg-result-actions"><button class="fg-result-primary" id="fgResultAgain"><i class="ti ti-refresh"></i> Jogar novamente</button><button class="fg-result-secondary" id="fgResultHome"><i class="ti ti-users"></i> Voltar à comunidade</button></div></div></div></section>';
  ...
  function answerMatch(match,state,q,answer,container){
    ...
    var currentWon=seals.length>=6,limitReached=!correct&&(state.rodada||1)>25,winnerId=null,winnerName=null;
