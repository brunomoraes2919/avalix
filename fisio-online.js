(function () {
  ...
  function musicBeat(){if(!audioActive||!audioState.music)return;var i=audioState.musicStep;if(audioState.mode==='victory'){var happy=[523,659,784,1047,784,880,988,1175],hb=[261.6,329.6,392,329.6,293.7,370,440,370];tone(hb[i%8],.3,'square',.035,0);tone(happy[i%8],.22,'triangle',.065,.02);if(i%2===0)tone(happy[(i+2)%8]*2,.07,'sine',.025,.04);audioState.musicStep++;return;}if(audioState.mode==='defeat'){var sad=[392,349,330,294,262,294,247,220],sb=[196,175,165,147];tone(sb[Math.floor(i/2)%4],.88,'sine',.03,0);tone(sad[i%8],.58,'triangle',.045,.05);audioState.musicStep++;return;}if(audioState.mode==='match'){var bass=[130.8,130.8,146.8,164.8,123.5,123.5,146.8,164.8],lead=[523,659,784,659,587,698,880,698];tone(bass[i%8],.32,'square',.032,0);tone(lead[i%8],.18,'triangle',.045,.025);if(i%2===0)tone(1046,.045,'square',.018,.02);audioState.musicStep++;return;}var chords=[[261.6,329.6,392],[293.7,370,440],[246.9,311.1,392],[220,277.2,329.6]],ch=chords[Math.floor(i/4)%chords.length],mel=[523,587,659,784,659,587,523,440,494,587,659,740,659,587,494,440][i%16];ch.forEach(function(n,j){tone(n,.78,'sine',.028,j*.018);});tone(mel,.25,'triangle',.038,.03);audioState.musicStep++;}
  function startMusic(){if(!audioActive)return;audioContext();if(!audioState.musicTimer){musicBeat();var speed=audioState.mode==='victory'?300:audioState.mode==='match'?360:audioState.mode==='defeat'?760:900;audioState.musicTimer=setInterval(musicBeat,speed);}}
  ...
  var FALLBACK_QUESTIONS=GAME_QUESTIONS.slice(), claimedQuestionIds={}, reportedQuestionIds={};
  var GAME_QUESTIONS = [
    ...
    {id:'s1',cat:'saude',q:'Na Atenção Primária à Saúde, uma ação coerente com a atuação fisioterapêutica coletiva é:',a:['Restringir-se ao atendimento hospitalar','Desenvolver ações de promoção e prevenção','Atuar apenas após cirurgias','Substituir a equipe multiprofissional'],c:1,e:'Na Atenção Primária, a fisioterapia pode atuar em promoção da saúde, prevenção de agravos e cuidado coletivo.'},
    {id:'s2',cat:'saude',q:'A educação em saúde realizada com a comunidade busca principalmente:',a:['Aumentar a dependência do serviço','Favorecer autonomia e práticas saudáveis','Eliminar o trabalho em equipe','Substituir toda avaliação individual'],c:1,e:'A educação em saúde busca fortalecer autonomia, participação e práticas de cuidado.'},
    {id:'h1',cat:'hospitalar',q:'No ambiente hospitalar, a mobilização precoce busca principalmente:',a:['Prolongar a imobilidade','Reduzir perdas funcionais associadas à internação','Evitar avaliação multiprofissional','Manter sedação sem indicação'],c:1,e:'A mobilização precoce auxilia na prevenção de fraqueza e perdas funcionais durante a internação.'},
    {id:'h2',cat:'hospitalar',q:'A higienização das mãos antes e após o contato com o paciente contribui diretamente para:',a:['Aumentar força muscular','Prevenir infecções relacionadas à assistência','Medir amplitude articular','Avaliar equilíbrio'],c:1,e:'A higiene das mãos é uma medida central de prevenção da transmissão de microrganismos no hospital.'},
    {id:'e1',cat:'esportiva',q:'Em uma lesão esportiva, o retorno ao esporte deve ser orientado principalmente por:',a:['Apenas o tempo transcorrido','Critérios funcionais e avaliação clínica','Preferência do atleta isoladamente','Ausência de qualquer exercício'],c:1,e:'O retorno seguro deve considerar critérios funcionais, avaliação clínica e demandas específicas do esporte.'},
    {id:'e2',cat:'esportiva',q:'O treinamento neuromuscular pode contribuir para prevenir lesões por melhorar:',a:['Somente a massa corporal','Controle motor, força e estabilidade','Apenas a frequência respiratória','Exclusivamente a flexibilidade passiva'],c:1,e:'Programas neuromusculares trabalham controle motor, força, equilíbrio e estabilidade relevantes à prevenção.'}
  ];
  ...
  function pickQuestion(used) {
    used=used||[]; var available=GAME_QUESTIONS.filter(function(q){return used.indexOf(q.id)<0&&!claimedQuestionIds[q.id];});
    if(!available.length) available=GAME_QUESTIONS.filter(function(q){return used.indexOf(q.id)<0;});
  ...
  function pickQuestionByCategory(category,used){
    used=used||[];var available=GAME_QUESTIONS.filter(function(q){return q.cat===category&&used.indexOf(q.id)<0&&!claimedQuestionIds[q.id];});
    if(!available.length)available=GAME_QUESTIONS.filter(function(q){return q.cat===category;});
    var q=available[Math.floor(Math.random()*available.length)]||pickQuestion(used);reserveQuestion(q);return q;
  }
  function reserveQuestion(q,matchId){
    if(!q)return;q.id=String(q.id);claimedQuestionIds[q.id]=true;
    if(!matchId||!q.remote||reportedQuestionIds[q.id]||!current())return;
    reportedQuestionIds[q.id]=true;
    api('/rest/v1/rpc/avalix_registrar_pergunta_usada',{method:'POST',body:JSON.stringify({p_usuario_id:String(current().id),p_pergunta_id:q.id,p_tema:q.cat,p_partida_id:matchId||null})}).catch(function(){delete reportedQuestionIds[q.id];});
  }
  function loadQuestionBank(){
    var s=current();if(!s)return Promise.resolve(GAME_QUESTIONS);
    return api('/rest/v1/rpc/avalix_perguntas_disponiveis',{method:'POST',body:JSON.stringify({p_usuario_id:String(s.id),p_limite:800})}).then(function(rows){
      ...
      if(!remote.length)return GAME_QUESTIONS;
      var covered={};remote.forEach(function(q){covered[q.cat]=true;});GAME_QUESTIONS=remote.concat(FALLBACK_QUESTIONS.filter(function(q){return !covered[q.cat];}));return GAME_QUESTIONS;
    }).catch(function(){return GAME_QUESTIONS;});
  }
  function turnDeadline(){return new Date(Date.now()+36*60*60*1000).toISOString();}

  function initialState(firstTurn) {
    var q=pickQuestion([]);
    return {version:3,turno:firstTurn,questao_id:q.id,questao:q,usadas:[q.id],selos_1:[],selos_2:[],acertos_1:0,acertos_2:0,sequencia_1:0,sequencia_2:0,rodada:1,jogada:1,coroa_pendente:false,modo_coroa:false,prazo_turno:turnDeadline(),ultima_resposta:null};
  }
  ...
  function respondInvite(id, accept) {
    ...
    api('/rest/v1/avalix_convites?id=eq.' + encodeURIComponent(id), {
      ...
      return loadQuestionBank().then(function(){return api('/rest/v1/avalix_partidas', {
  ...
  function renderMatchWheel(match,state,q,category,mine,mySeals,otherSeals,otherName) {
    ...
    function spin(){
      ...
      requestAnimationFrame(function(){wheel.style.transform='rotate('+(2520-index*40)+'deg)';});
  ...
  function renderCrownChoice(match,state){
    ...
    container.querySelectorAll('[data-crown-cat]').forEach(function(btn){btn.onclick=function(){
      var cat=btn.dataset.crownCat,q=pickQuestionByCategory(cat,state.usadas||[]);state.questao_id=q.id;state.questao=q;state.usadas=(state.usadas||[]).concat([q.id]);state.coroa_pendente=false;state.modo_coroa=true;state.categoria_coroa=cat;
  ...
  function renderMatch(match) {
    ...
    if (!container) return;
    var savedState=typeof match.estado==='string'?JSON.parse(match.estado):match.estado||{};
    if(savedState.questao&&!GAME_QUESTIONS.some(function(item){return String(item.id)===String(savedState.questao.id);}))GAME_QUESTIONS.push(savedState.questao);
    ...
    if(Date.now()>deadline){
      ...
      state.coroa_pendente=false;state.modo_coroa=false;state.categoria_coroa=null;state.rodada=(state.rodada||1)+1;state.jogada=(state.jogada||1)+1;state.questao_id=nqExpired.id;state.questao=nqExpired;state.usadas=(state.usadas||[]).concat([nqExpired.id]);state.prazo_turno=turnDeadline();state.ultima_resposta={por:expiredId,correta:false,tempo_turno_esgotado:true,em:new Date().toISOString()};
    ...
    if(myTurn)reserveQuestion(q,match.id);
  ...
  function answerMatch(match,state,q,answer,container){
    ...
    var currentWon=seals.length>=8,limitReached=!correct&&(state.rodada||1)>25,winnerId=null,winnerName=null;
    ...
    state.usadas=(state.usadas||[]).concat([nq.id]);state.questao_id=nq.id;state.questao=nq;state.jogada=(state.jogada||1)+1;state.prazo_turno=correct?(state.prazo_turno||turnDeadline()):turnDeadline();state.ultima_resposta={por:s.id,correta:correct,questao:q.id,tempo_esgotado:answer<0,em:new Date().toISOString()};
  ...
  function start() {
    ...
    heartbeat(false); loadQuestionBank(); loadPresence(); loadInvites(); loadMatches(); loadChatNotifications(); loadGameProfile(); loadRanking();
