(function () {
  ...
  function renderMatchWheel(match,state,q,category,mine,mySeals,otherSeals,otherName) {
    ...
    function spin(){
      ...
      if(button.disabled)return; button.disabled=true; center.disabled=true; shell.classList.add('spinning'); result.innerHTML='<i class="ti ti-loader-2"></i><span>Sorteando área...</span>';
      ...
      setTimeout(function(){shell.classList.remove('spinning');shell.classList.add('landed');result.style.setProperty('--result',category.color);result.innerHTML='<i class="ti '+icons[index]+'"></i><span>'+esc(category.name)+'</span>';},3700);
