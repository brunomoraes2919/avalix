(function () {
  ...
  var GAME_CATEGORIES = [
    {id:'anatomia',name:'Anatomia',color:'#d96b5f',mascot:'mascote-anatomia.png'}, {id:'cinesio',name:'Cinesiologia',color:'#d8a83e',mascot:'mascote-cinesio.png'},
    {id:'orto',name:'Ortopedia',color:'#3b82c4',mascot:'mascote-orto.png'}, {id:'neuro',name:'Neurologia',color:'#8858b5',mascot:'mascote-neuro.png'},
    {id:'cardio',name:'Cardiorrespiratória',color:'#d74c72',mascot:'mascote-cardio.png'}, {id:'saude',name:'Saúde Coletiva',color:'#338f75',mascot:'mascote-saude.png'},
    {id:'hospitalar',name:'Hospitalar',color:'#5869b2',mascot:'mascote-hospitalar.png'}, {id:'esportiva',name:'Fisioterapia Esportiva',color:'#df7b32',mascot:'mascote-esportiva.png'}
  ];
  var GAME_QUESTIONS = [
  ...
  function categoryMascot(category, className) {
    if (!category || !category.mascot) return '';
    return '<img class="'+(className||'fg-category-mascot')+'" src="'+esc(category.mascot)+'?v=57" alt="Mascote de '+esc(category.name)+'" loading="lazy">';
  }

  function worldFacts() {
    var list=window.FISIOGAME_FACTS;
    return Array.isArray(list)&&list.length?list:[
      'A fisioterapia utiliza o movimento como ferramenta para ampliar funcionalidade e participação.',
      'A reabilitação pode ser necessária em qualquer fase da vida.',
      'Fisioterapeutas atuam em hospitais, comunidades, escolas, esportes e cuidados domiciliares.'
    ];
  }

  function nextWorldFact(facts) {
    var key='fg_world_fact_deck_v60',deck=[],last=-1;
    try{var saved=JSON.parse(localStorage.getItem(key)||'{}');deck=Array.isArray(saved.deck)?saved.deck:[];last=Number(saved.last);}
    catch(e){deck=[];}
    deck=deck.filter(function(index,pos){return Number.isInteger(index)&&index>=0&&index<facts.length&&deck.indexOf(index)===pos;});
    if(!deck.length){
      deck=facts.map(function(_,index){return index;});
      for(var i=deck.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1)),swap=deck[i];deck[i]=deck[j];deck[j]=swap;}
      if(deck.length>1&&deck[0]===last){var first=deck[0];deck[0]=deck[1];deck[1]=first;}
    }
    var index=deck.shift();
    try{localStorage.setItem(key,JSON.stringify({deck:deck,last:index}));}catch(e){}
  ...
  function renderMatchWheel(match,state,q,category,mine,mySeals,otherSeals,otherName) {
    ...
    var wheelItems=GAME_CATEGORIES.map(function(c,i){return '<span style="--i:'+i+'">'+categoryMascot(c,'fg-wheel-mascot')+'<small>'+esc(c.name.split(' ')[0])+'</small></span>';}).join('')+'<span class="fg-crown-slice" style="--i:8"><i class="ti ti-crown"></i><small>Coroa</small></span>';
    ...
    function spin(){
      ...
      setTimeout(function(){sound('land');shell.classList.remove('spinning');shell.classList.add('landed');result.style.setProperty('--result',crown?'#e8b83f':category.color);result.innerHTML=crown?'<i class="ti ti-crown"></i><span>COROA! Escolha uma área</span>':categoryMascot(category,'fg-result-mascot')+'<span>'+esc(category.name)+'</span>';},3700);
  ...
  gameStyles += '.fg-wait-player{position:relative;width:min(720px,100%);min-height:230px;gap:54px;padding:27px 34px 27px 45px;background:radial-gradient(circle at 16% 50%,rgba(54,145,124,.13),transparent 31%),linear-gradient(135deg,#fff8dc,#fff);overflow:visible}.fg-wait-photo-orbit{width:138px;height:138px;flex:0 0 138px;margin:0!important;background:transparent!important;box-shadow:none!important;font-size:inherit!important}.fg-wait-photo-orbit>.fg-duel-photo{position:relative;z-index:2;width:104px!important;height:104px!important;flex:0 0 104px;font-size:28px!important}.fg-wait-photo-orbit>b{inset:4px;border-color:rgba(47,150,127,.32);z-index:1}.fg-wait-photo-orbit>b:nth-of-type(2){inset:-24px}.fg-wait-photo-orbit>b:nth-of-type(3){inset:-52px}.fg-wait-photo-orbit>b:after{width:9px;height:9px}.fg-wait-photo-orbit>b:nth-of-type(1):after{left:13%;top:7%}.fg-wait-photo-orbit>b:nth-of-type(2):after{left:auto;right:-4px;top:48%}.fg-wait-photo-orbit>b:nth-of-type(3):after{left:31%;top:auto;bottom:-4px}.fg-wait-player>div:last-child{position:relative;z-index:2}.fg-wait-player>div:last-child>strong{max-width:480px}@media(max-width:600px){.fg-wait-player{min-height:350px;gap:42px;padding:37px 18px 24px;overflow:hidden}.fg-wait-photo-orbit{width:126px;height:126px;flex-basis:126px}.fg-wait-photo-orbit>.fg-duel-photo{width:92px!important;height:92px!important;flex-basis:92px}.fg-wait-photo-orbit>b:nth-of-type(2){inset:-20px}.fg-wait-photo-orbit>b:nth-of-type(3){inset:-44px}}';
  ...
  progressStyles += '.fg-mastery-mascot{width:34px;height:34px;object-fit:contain;filter:drop-shadow(0 3px 4px rgba(24,59,53,.18))}.fg-collection-title{margin-bottom:4px}.fg-collection-copy{margin:0 0 14px;color:var(--ac-charcoal-soft);font-size:12px}.fg-area-collection{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.fg-area-card{position:relative;overflow:hidden;min-width:0;border:2px solid color-mix(in srgb,var(--area) 35%,#dfe9e5);border-radius:18px;padding:12px 8px 11px;text-align:center;display:grid;justify-items:center;gap:5px;background:linear-gradient(145deg,color-mix(in srgb,var(--area) 8%,#fff),#fff);box-shadow:0 6px 0 color-mix(in srgb,var(--area) 22%,#d6e2de)}.fg-area-card>div{position:relative;width:100%;height:112px;display:grid;place-items:center}.fg-area-mascot{width:108px;height:108px;object-fit:contain;filter:drop-shadow(0 7px 8px rgba(22,57,51,.2));transition:.3s}.fg-area-card>div>i{position:absolute;right:6px;top:5px;width:25px;height:25px;border-radius:50%;display:grid;place-items:center;background:#fff;color:var(--area);box-shadow:0 4px 10px rgba(0,0,0,.14)}.fg-area-card b{font-size:12px}.fg-area-card span{font-size:7px;font-weight:900;letter-spacing:.08em;color:var(--area)}.fg-area-card.locked .fg-area-mascot{filter:grayscale(1) brightness(.72);opacity:.38}.fg-area-card.locked{background:#f4f6f5;border-color:#dce4e1;box-shadow:0 6px 0 #d7e0dd}.fg-area-card.unlocked:hover .fg-area-mascot{transform:translateY(-5px) scale(1.05)}@media(max-width:760px){.fg-area-collection{grid-template-columns:1fr 1fr}.fg-area-card>div{height:96px}.fg-area-mascot{width:94px;height:94px}}';
...
.fg-shell{display:grid;gap:18px;max-width:1180px;margin:0 auto}.fg-hero{background:linear-gradient(135deg,#173f3a,#246b5f);color:#fff;border-radius:22px;padding:clamp(22px,4vw,42px);display:flex;align-items:end;justify-content:space-between;gap:24px;box-shadow:0 16px 40px rgba(23,63,58,.18)}.fg-hero h2{font-size:clamp(26px,4vw,44px);line-height:1.06;max-width:720px;margin:8px 0 10px;color:#fff}.fg-hero p{margin:0;color:rgba(255,255,255,.78);max-width:650px}.fg-kicker{font:700 11px/1.2 Inter,sans-serif;letter-spacing:.16em;color:#e7bf72}.fg-community,.fg-invites{background:var(--ac-white,#fff);border:1px solid rgba(30,50,45,.1);border-radius:18px;padding:clamp(16px,3vw,26px)}.fg-section-head{display:flex;align-items:end;justify-content:space-between;gap:18px;margin-bottom:14px}.fg-section-head h3,.fg-invites h3{margin:3px 0 0}.fg-search{display:flex;align-items:center;gap:7px;border:1px solid rgba(30,50,45,.18);border-radius:10px;padding:0 10px;min-width:230px}.fg-search input{border:0!important;box-shadow:none!important;padding:9px 0!important;width:100%}.fg-legend{display:flex;gap:18px;flex-wrap:wrap;font-size:12px;color:var(--ac-charcoal-soft);margin-bottom:14px}.fg-dot,.fg-status{display:inline-block;width:9px;height:9px;border-radius:50%;margin-right:5px}.fg-dot.on,.fg-status.on{background:#27ae60;box-shadow:0 0 0 3px rgba(39,174,96,.14)}.fg-dot.off,.fg-status.off{background:#a9b1ad}.fg-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.fg-person{display:flex;align-items:center;gap:12px;border:1px solid rgba(30,50,45,.09);border-radius:14px;padding:12px;background:#fff}.fg-avatar{width:46px;height:46px;min-width:46px;min-height:46px;max-width:46px;max-height:46px;aspect-ratio:1/1;border-radius:50%;background:#e8f0ed;color:#1b594f;display:grid;place-items:center;font-weight:800;position:relative;flex:0 0 46px;overflow:visible}.fg-avatar img{display:block;position:absolute;inset:0;width:46px!important;height:46px!important;min-width:46px;min-height:46px;max-width:46px;max-height:46px;aspect-ratio:1/1;border-radius:50%!important;object-fit:cover!important;object-position:center}.fg-status{position:absolute;right:-1px;bottom:1px;margin:0;border:2px solid #fff;width:11px;height:11px}.fg-person-copy{display:grid;min-width:0;flex:1}.fg-person-copy b{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.fg-person-copy span{font-size:12px;color:var(--ac-charcoal-soft)}.fg-invites{border-color:rgba(231,191,114,.5);background:#fffdf7}.fg-invite{display:flex;justify-content:space-between;align-items:center;gap:16px;padding:12px 0;border-top:1px solid rgba(30,50,45,.08)}.fg-invite>div:first-child{display:grid}.fg-invite span{font-size:13px;color:var(--ac-charcoal-soft)}.fg-invite>div:last-child{display:flex;gap:7px}.fg-empty{text-align:center;padding:32px;color:var(--ac-charcoal-soft);grid-column:1/-1}.fg-game-frame{height:calc(100vh - 118px);min-height:620px;display:flex;flex-direction:column}.fg-framebar{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:0 0 10px;font-size:12px;color:var(--ac-charcoal-soft)}.fg-game-frame iframe{width:100%;flex:1;border:0;border-radius:18px;background:#f4f1e9}.fg-match{max-width:680px;margin:50px auto;text-align:center;background:#fff;border-radius:22px;padding:48px 28px;border:1px solid rgba(30,50,45,.1)}.fg-match-icon{width:74px;height:74px;border-radius:50%;display:grid;place-items:center;margin:0 auto 18px;background:#e7f1ee;color:#1f655a;font-size:36px}.fg-match h2{margin:9px 0}.fg-match p{color:var(--ac-charcoal-soft);margin-bottom:24px}@media(max-width:760px){.fg-hero{align-items:stretch;flex-direction:column}.fg-hero .btn{width:100%}.fg-grid{grid-template-columns:1fr}.fg-section-head{align-items:stretch;flex-direction:column}.fg-search{min-width:0}.fg-person{padding:10px}.fg-invite{align-items:flex-start;flex-direction:column}.fg-game-frame{height:calc(100vh - 92px);min-height:520px}.fg-framebar span{display:none}}';
  ...
  gameStyles += '.fg-wheel-mascot{position:absolute;top:4%;width:42px;height:42px;object-fit:contain;transform:rotate(calc(var(--i)*-40deg));filter:drop-shadow(0 3px 4px rgba(0,0,0,.3));z-index:2}.fg-wheel>span small{top:18%!important}.fg-result-mascot{width:43px;height:43px;object-fit:contain;filter:drop-shadow(0 4px 5px rgba(0,0,0,.24));margin:-6px 1px}.fg-crown-mascot{width:82px;height:82px;object-fit:contain;filter:drop-shadow(0 6px 7px rgba(23,57,51,.2));margin-bottom:4px}.fg-crown-choice button>i:empty{display:none}.fg-seal-board>div{position:relative;min-height:78px;overflow:hidden;padding-top:6px!important}.fg-seal-mascot{width:54px;height:54px;object-fit:contain;filter:grayscale(1);opacity:.3}.fg-seal-board>div.got .fg-seal-mascot{filter:none;opacity:1;transform:scale(1.08);filter:drop-shadow(0 5px 6px color-mix(in srgb,var(--seal) 35%,transparent))}.fg-seal-board>div>i{position:absolute;right:5px;top:5px;width:18px;height:18px;border-radius:50%;display:grid;place-items:center;background:rgba(255,255,255,.88);font-size:11px;z-index:3}.fg-seal-board>div>span{font-size:6px!important}.fg-question-meta>span{display:flex!important;align-items:center;gap:7px}.fg-question-mascot{width:42px;height:42px;object-fit:contain;filter:drop-shadow(0 4px 5px rgba(20,50,45,.18))}@media(max-width:600px){.fg-wheel-mascot{width:34px;height:34px}.fg-crown-mascot{width:68px;height:68px}.fg-seal-mascot{width:46px;height:46px}.fg-seal-board>div{min-height:69px}}';
